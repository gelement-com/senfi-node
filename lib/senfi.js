...
const { ProxyConnect } = require('./ProxyConnect');
...
Senfi.prototype = {
   ...
   * @param {object} [config.proxyConnectConfig] - Configuration options for ProxyConnect
  ...
  initialize(key, secret, config = {}) {
    ...
    ProxyConnect.init(props.proxyConnectConfig);
  ...
  _authenticate(callback) {
    ...
    // build request
    const axiosAgent = ProxyConnect.axiosAgent({ baseURL: baseUrl });
    let options = {
      baseURL: baseUrl,
      url: '/token',
      method: 'post',
