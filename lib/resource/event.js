...
Event.prototype = {
  ...
  subscribe(subscriptionOptions) {
    return new Promise((resolve, reject) => {
      ...
      if (values.length > 0) {
        reject({
          success: false,
          errmsg: 'Invalid arguments. Unexpected properties in input: ' + values.join(', '),
          errcode: 'invalid_argument'
        })
        return;
      }
   ...
   * @param {object} [connectorParams]
   * @param {string} [connectorParams.connector_id]
   * @param {string} [connectorParams.instance_id]
   * @return {Promise<EventGenerateResponse>}
   */
  generate(eventDefId, type, inputs, eventParams, connectorParams) {
    return new Promise((resolve, reject) => {
      ...
      switch (type) {
        case 'measurement': {
          if (typeof eventParams !== 'object' || eventParams === null) {
            reject({
              success: false,
              errmsg: 'Invalid arguments. Fourth argument should be an object.',
              errcode: 'invalid_argument'
            })
            return;
          }
          ...
          if (!Array.isArray(eventParams.metric_caused) || !Array.isArray(eventParams.measurement_snapshot)) {
            reject({
              success: false,
              errmsg: 'Invalid arguments. "metric_caused" and "measurement_snapshot" should be arrays.',
              errcode: 'invalid_argument'
            })
            return;
          }
        ...
        case 'external': {
          ...
          if (!Array.isArray(eventParams.tag_data)) {
            reject({
              success: false,
              errmsg: 'Invalid arguments. "tag_data" should be an array.',
              errcode: 'invalid_argument'
            })
            return;
          }
        ...
        default: {
          reject({
            success: false,
            errmsg: 'Invalid arguments. Unknown "type" value: ' + type + '.',
            errcode: 'invalid_argument'
          });
          return;
        }
      ...
      if (typeof connectorParams === 'object' && connectorParams !== null) {
        // Check for any unexpected values
        const ALLOWED_ATTRIBUTE_PROPERTIES = ['connector_id', 'instance_id'];
        const values = Object.keys(connectorParams).filter(
          (value) => !ALLOWED_ATTRIBUTE_PROPERTIES.includes(value)
        );
        if (values.length > 0) {
          reject({
        ...
        options.connector_id = connectorParams.connector_id;
        options.instance_id = connectorParams.instance_id;
      }
