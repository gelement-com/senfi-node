...
Asset.prototype = {
   ...
   * @param {Array.{integer}|integer} filter
   */
  getDetail(filter) {
    return new Promise((resolve, reject) => {
      ...
      try {
        this._senfi.httpRequest('get', '/asset/detail', filter, function(res) {
          resolve(res);
        });
      } catch (ex) {
        reject({
  ...
  getAttributeValue(assetParam, attributeName, locale) {
    return new Promise((resolve, reject) => {
      // Check for any unexpected values
      const ALLOWED_ATTRIBUTE_PROPERTIES = ['asset_id', 'measurement_code', 'asset_tag'];
      const values = Object.keys(assetParam).filter(
        (value) => !ALLOWED_ATTRIBUTE_PROPERTIES.includes(value)
      );
      if (values.length > 0) {
