...
Asset.prototype = {
  ...
  /**
   * Retrieve details of the specified assets by asset_id
   * @param {object} assetParam
   * @param {Array.<integer>|integer} assetParam.asset_id
   * @return {Promise<AssetResponse>}
   */
  ...
  /**
   * @typedef {object} AssetAttributeResponse
   * @param {boolean} success
   * @param {string} [value]
   * @param {string} [errmsg]
   * @param {string} [errcode]
   ...
   * @param {string} [locale]
   * @return {Promise<AssetAttributeResponse>}
   */
  getAttributeValue(assetParam, attributeName, locale) {
    return new Promise((resolve, reject) => {
      ...
      if (typeof attributeName !== 'string') {
        reject({
          success: false,
          errmsg: 'Invalid arguments. Second argument should be a string.',
          errcode: 'invalid_argument'
        })
        return;
      }
